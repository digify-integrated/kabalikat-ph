...
function setNavActive() {
    ...
    document.querySelectorAll('.side-menu__item[href]').forEach((e) => {
        const href = e.getAttribute('href');

        if (!href || href === 'javascript:void(0);') return;

        const itemPath = href.replace(/^\/+/, '').split('/').pop();
        ...
        if (itemPath === currentPath) {
            ...
            // 🔑 THIS IS THE CRITICAL LOOP (your logic)
            while (parentUl && parentUl.previousElementSibling) {
                const parentLink = parentUl.previousElementSibling;
                const parentLi = parentLink.parentElement;

                parentLink.classList.add('active', 'open');
