...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function generateKitchenTickets(
        ...
        try {
            ...
            */
            ...
            /*
            |--------------------------------------------------------------------------
            | RENDER HEADER + STATS + TICKETS
            |--------------------------------------------------------------------------
            */

            container.innerHTML =
                renderKitchenDisplay(route);
    ...
    function renderKitchenTicket(ticket) {
        ...
        // Dynamic border class: adds a thick red left-border if delayed
        const borderClass = isDelayed ? 'border-start border-danger border-3' : 'border-0';
        // Dynamic text class for the timer
        ...
        <div class="card shadow-sm ${borderClass}">
            ...
            <div class="card-header py-2">
                ...
                <div class="d-flex justify-content-between align-items-center w-100">
                    ...
                    <div class="text-end">
                        <span class="badge ${getTicketStatusBadge(ticket.ticket_status)}">
    ...
    function renderKitchenItem(item, index, totalItems, ticketStatus) {
        ...
        const base = Number(item.base_quantity ?? remaining);
        
        const isCancelled = item.status === 'Cancelled' || Number(item.cancelled_quantity ?? 0) > 0;
        
        /*
        |--------------------------------------------------------------------------
        | DYNAMIC CHECKLIST PROGRESSION LOOKUP
        |--------------------------------------------------------------------------
        | An item displays a strike-through if its single status runs ahead 
        | of the current active Column Board pipeline state container layout.
        */
        let isDone = false;
        ...
        >
            <div class="d-flex align-items-center gap-3">
                <div class="text-center" style="min-width: 45px;">
                    <span class="fs-5 fw-extrabold ${textDecorationClass}">
                        ${remaining}x
                    </span>
                </div>
                <div>
                    <div class="fw-bold fs-6 ${textDecorationClass}">
                        ${item.product_name}
                    </div>
                    <div class="text-muted small mt-0.5">
                        Total Ordered: ${base}x
                    </div>
                    ${item.note ? `
            ...
            <div class="text-end ps-2">
                ${isDone ? `
                    <span class="badge bg-success text-white px-2 py-1.5 d-flex align-items-center gap-1 shadow-sm">
                        ✅ Done
                    </span>
                ` : isCancelled ? `
                    <span class="badge bg-danger text-white px-2 py-1.5 small">
                        VOID
                    </span>
                ` : `
                    <span class="badge bg-warning text-dark border px-2 py-1.5 text-uppercase tracking-wider small fw-bold">
                        TO DO
                    </span>
                `}
            </div>
    ...
    document.addEventListener('click', async (event) => {
        ...
        try {
            ...
            await generateKitchenTickets('/kitchen-ticket/generate-kitchen-tickets');
