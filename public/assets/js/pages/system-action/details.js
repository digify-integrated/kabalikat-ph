...
import { disableButton, enableButton, detailsDeleteButton, detailsTableActionButton, permissionToggle } from '../../form/button.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    (async () => {
        try {
            ...
            // 3) Start nav details immediately
            ...
            // 5) Await everything together (concurrent)
            await Promise.all([
                optionsPromise,
                navDetailsPromise,
                routeDetailsPromise,
    ...
    initValidation('#role_permission_assignment_form', {
        submitHandler: async (form) => {
            const formData = new URLSearchParams(new FormData(form));
            formData.append('navigation_menu_id', ctx.detailId ?? '');
            formData.append('appId', ctx.appId ?? '');
    ...
    attachLogNotesClassHandler('.view-role-permission-log-notes', 'role_permission');
